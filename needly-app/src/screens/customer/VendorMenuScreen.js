...
export default function VendorMenuScreen({ route, navigation }) {
  ...
  if (!vendor) {
    return (
      <View style={{ flex: 1, backgroundColor: COLORS.paper, padding: 20, alignItems: "center", justifyContent: "center" }}>
        <Text style={{ fontSize: 42, marginBottom: 10 }}>🏪</Text>
        <Text style={{ fontSize: 18, fontWeight: "800", color: COLORS.ink, marginBottom: 6 }}>Vendor Not Found</Text>
        <Text style={{ fontSize: 13, color: COLORS.mute, textAlign: "center", marginBottom: 16 }}>
          This vendor storefront may have been updated or is temporarily unavailable.
        </Text>
  ...
  const subcategories = hasSubcategories ? ["All", ...new Set(items.map((i) => i.subcategory).filter(Boolean))] : [];
  const displayItems = subFilter === "All" ? items : items.filter((i) => i.subcategory === subFilter);
  const sections = groupBySubcategory(displayItems);

  const add = (id) => setCart((c) => ({ ...c, [id]: (c[id] || 0) + 1 }));
  const remove = (id) => setCart((c) => ({ ...c, [id]: Math.max(0, (c[id] || 0) - 1) }));
  const count = Object.values(cart).reduce((a, b) => a + b, 0);
  const total = items.reduce((s, i) => s + (cart[i.id] || 0) * (i.price || 0), 0);
      ...
      <View style={{ padding: 16, paddingBottom: 0 }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 6 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 12, color: COLORS.mute }}>★ {vendor.rating || 4.5} · {vendor.category}</Text>
          </View>
        </View>

        <View style={{ flexDirection: "row", gap: 8, alignItems: "center", marginBottom: 10 }}>
          <Pill tone="indigo">{vendor.area || "Abeokuta"}</Pill>
          <Text style={{ fontSize: 12.5, color: COLORS.mute }}>{vendor.eta || "20-35 min"}</Text>
        </View>
      ...
      <SectionList
        ...
        ListEmptyComponent={
          <View style={{ padding: 20, alignItems: "center" }}>
            <Text style={{ color: COLORS.mute, fontSize: 13.5 }}>No items available in this catalogue section.</Text>
          </View>
        }
...
const styles = StyleSheet.create({
  title: { fontWeight: "800", fontSize: 19, color: COLORS.ink, marginBottom: 2 },
  backBtn: { backgroundColor: COLORS.ink, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 14 },
